...
function ClassTimer:OnInitialize()
	...
	for k, v in pairs(table) do
		...
		local k = L[k]
		--k: Misc, DOTs, Stuns, BUffs, Race
		-- these are some of the tabs in the options
		timerargs[k] = {
			type = "group",
			name = k,
...
function ClassTimer:UNIT_AURA(units)	
	for unit in pairs(units) do
		--print(unit)  --nameplate1 to 4, player
	...
	function ClassTimer:GetBuffs(unit, db)
		...
		if db.buffs then
            ...
            while true do
				...
				if duration and (self.db.profile.Abilities[name] or self.db.char.Custom[name]) and isMine or self.db.char.AlwaysShown[name] then
					--DevTools_Dump(C_UnitAuras.GetBuffDataByIndex(unit, i))
					...
					--print("name: "..tostring(self.db.profile.Abilities[name]))
					--print("Custom: "..tostring(self.db.char.Custom[name]))
					--print("isMine: "..tostring(isMine))
					--print("alwaysshown: "..tostring(self.db.char.AlwaysShown[name]))
					--print("======================")
					local t = new()
		...
		if db.debuffs then
			...
			while true do
				...
				local count =  auradata.applications
				local duration = auradata.duration
				local debuffType = auradata.dispelName --not sure
				local endTime = auradata.expirationTime
				local texture = auradata.icon
				local name = auradata.name
				local caster = auradata.sourceUnit
